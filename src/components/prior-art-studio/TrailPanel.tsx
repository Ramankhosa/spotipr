  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        <span className="text-[11px] font-semibold uppercase tracking-widest text-muted-foreground">Evidence trail</span>
        <Hint
      ...
      <ol className="space-y-0 divide-y divide-border rounded-lg border border-border bg-card px-3">
        ...
        {entries.map(entry => (
          <li key={entry.id} className="flex items-baseline gap-2.5 py-2 text-xs">
            ...
            <span className={`shrink-0 rounded px-1.5 py-0.5 font-mono text-[9px] font-bold ${KIND_STYLES[entry.kind] || KIND_STYLES.SYSTEM}`}>
              {entry.kind}
            </span>
