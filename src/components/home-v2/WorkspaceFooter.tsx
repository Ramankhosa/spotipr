...
export default function WorkspaceFooter() {
  return (
    <footer className="mt-24 border-t border-vellum-900 bg-vellum-100 lg:mt-28">
      <div className="mx-auto max-w-[1240px] px-5 py-14 sm:px-8">
        <div className="grid gap-10 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,2.4fr)_minmax(0,0.7fr)]">
          <div>
            <div className="flex items-center gap-2 text-lamp-600">
              ...
              <span className="text-[18px] font-semibold tracking-[-0.02em] text-vellum-900">
                Patent<span className="text-lamp-600">Nest</span>
              </span>
            </div>
            <p className="mt-3 text-[12.5px] leading-[1.6] text-vellum-500">
          ...
          <div className="grid grid-cols-2 gap-8 sm:grid-cols-4">
            {COLUMNS.map((col) => (
              <div key={col.heading}>
                <p className="mb-3 text-[10.5px] font-medium uppercase tracking-[0.14em] text-vellum-500">
                  {col.heading}
                </p>
                <ul className="space-y-2">
                  {col.links.map((l) => (
                    <li key={l.label}>
                      <Link
                        href={l.href}
                        className="text-[13px] text-vellum-700 transition-colors hover:text-lamp-600"
                      >
                        {l.label}
                      </Link>
                    </li>
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              <a
                ...
                aria-label="LinkedIn"
                className="flex h-9 w-9 items-center justify-center border border-vellum-400 text-vellum-600 transition-colors hover:border-lamp-300 hover:text-lamp-600"
              >
        ...
        <div className="mt-12 flex flex-col items-start justify-between gap-4 border-t border-vellum-400 pt-6 sm:flex-row sm:items-center">
          <nav className="flex flex-wrap items-center gap-x-6 gap-y-2">
            {LEGAL.map((l) => (
              <Link
                key={l.label}
                href={l.href}
                className="text-[12.5px] text-vellum-600 transition-colors hover:text-lamp-600"
              >
                {l.label}
              </Link>
            ))}
          </nav>
          <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-vellum-500">
            © {new Date().getFullYear()} PatentNest.ai
          </p>
        </div>
      </div>
