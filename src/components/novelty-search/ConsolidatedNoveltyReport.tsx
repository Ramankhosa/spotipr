...
import { buildNoveltyAttorneyReportModel, formatFirmAddressLines, type FirmBranding } from '@/lib/novelty-attorney-report';
...
function defaultProfessionalRemark(status: FeatureStatus, feature = '', patentDisclosure = ''): string {
  ...
  return `The reviewed citation does not establish ${feature || 'this feature'}, so it carries no claim weight against this point on the mapped evidence.`;
}
...
export default function ConsolidatedNoveltyReport({ searchId, searchData, readOnly = false, firm = null }: ConsolidatedNoveltyReportProps) {
    ...
    <>
      ...
      <div className="min-h-screen bg-slate-100 text-slate-950 print:bg-white">
        ...
        <main className="mx-auto max-w-6xl bg-white px-4 py-6 shadow-sm sm:px-6 lg:px-8 lg:py-10 print:max-w-none print:px-0 print:py-0 print:shadow-none">
          <header
            className="mb-10 border-b-4 border-ai-blue-700 pb-8"
            style={firm?.accentColor ? { borderBottomColor: firm.accentColor } : undefined}
          >
            <div className="flex flex-col justify-between gap-6 md:flex-row md:items-start">
              ...
              <div className="text-left text-sm text-slate-600 md:text-right">
                {firm ? (
                  <div className="md:flex md:flex-col md:items-end">
                    {firm.logoDataUri ? (
                      ...
                      <img src={firm.logoDataUri} alt={firm.firmName} className="mb-1 max-h-12 max-w-[200px] object-contain md:ml-auto" />
                    ) : (
                      <div className="text-xl font-bold text-slate-950">{firm.firmName}</div>
                    )}
                    {formatFirmAddressLines(firm).map((line, index) => (
                ...
                <div className="mt-2">Report ID: {(searchId ?? '').slice(0, 8)}</div>
          ...
          <Section id="section-1-7" title="1.7 Key Feature Analysis">
            <p className="mb-4 max-w-4xl text-sm leading-6 text-slate-700">
              Each cell records whether the cited patent record discloses the corresponding key feature, based on the mapped evidence for that reference.
            </p>
              ...
              <>
                ...
                {reportData.strategicReviewFocus && (
                  <div className="mt-5 rounded-sm border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-950">
                    ...
                    <p><span className="font-semibold">Priority references: </span>{reportData.strategicReviewFocus.recommendedFullTextReview.join(', ') || '-'}</p>
                  </div>
                )}
              </>
            ) : (
          ...
          <footer className="mt-12 border-t border-slate-300 pt-6 text-center text-xs leading-5 text-slate-500">
            {firm ? (
              <p>
                Prepared by {firm.firmName} - Confidential.
                {firm.showPoweredBy !== false ? ' Powered by PatentNest.ai.' : ''}
              </p>
            ) : (
              <p>Prepared by PatentNest.ai - Confidential.</p>
            )}
