    ...
    for (const comp of components as Comp[]) {
      ...
      nodes[id] = {
        ...
        claimSupport: (comp as any).claimSupport && typeof (comp as any).claimSupport === 'object' && !Array.isArray((comp as any).claimSupport)
          ...
          : undefined,
        autoGenerated: (comp as any).autoGenerated === true,
        autoGeneratedBy: typeof (comp as any).autoGeneratedBy === 'string' ? (comp as any).autoGeneratedBy : undefined,
        autoGeneratedAt: typeof (comp as any).autoGeneratedAt === 'string' ? (comp as any).autoGeneratedAt : undefined,
        requirementIds: Array.isArray((comp as any).requirementIds) ? (comp as any).requirementIds.filter((id: unknown) => typeof id === 'string') : undefined,
        claimNumbers: Array.isArray((comp as any).claimNumbers) ? (comp as any).claimNumbers.filter((number: unknown) => Number.isInteger(number)) : undefined,
        sourceText: typeof (comp as any).sourceText === 'string' ? (comp as any).sourceText : undefined,
        children: []
      };
    }
    ...
    if (numberingStyle === 'NUMERIC_BUCKET') {
      ...
      const collect = (n: any) => {
        processedComponents.push({
          ...
          autoGeneratedBy: n.autoGeneratedBy,
          autoGeneratedAt: n.autoGeneratedAt,
