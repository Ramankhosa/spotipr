...
function componentLines(components: PatentDiagramComponent[]): string {
  return components.map(component => {
    ...
    return `- id=${component.id}${reference}; name=${component.name}; type=${component.type || 'OTHER'}${parent}${detail}`
  }).join('\n')
}
