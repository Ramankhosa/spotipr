...
async function incrementBucket(
  ...
  const rows = await tx.$queryRaw<Array<{ requestCount: number }>>(Prisma.sql`
    ...
    VALUES
      (${crypto.randomUUID()}, ${clientId}, ${periodType}::"PatentApiUsagePeriod", ${periodStart}, 1, NOW(), NOW())
  ...
  try {
    ...
    if (error instanceof PatentApiError) {
      error.auth = {
        ...
        quota: {
          minute: { limit: client.rateLimitPerMinute, remaining: 0, resetAt: new Date(Date.now() + (error.retryAfter || 60) * 1000) },
          day: { limit: client.dailyRequestLimit, remaining: 0, resetAt: new Date(Date.now() + (error.retryAfter || 60) * 1000) },
          month: { limit: client.monthlyRequestLimit, remaining: 0, resetAt: new Date(Date.now() + (error.retryAfter || 60) * 1000) },
        },
      }
    }
    throw error
