...
// NOTE: the former `searchPatentCorpus()` legacy lane was deleted here. It
// hardcoded `::vector` / `<=>` and a similarity formula with no /DIMENSIONS
// term, so it could only ever have worked against the retired OpenAI float
// column. It had zero callers. Use the provider lane (patent-search/**) or
// element-scoring.ts, both of which derive column/operator/cast from the
// PATENT_CORPUS_EMBEDDING_* constants above.
