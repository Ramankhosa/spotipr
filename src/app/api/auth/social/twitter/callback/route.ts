...
import { parsePaidSignupStateFromObject } from '@/lib/oauth-state'
...
export async function GET(request: NextRequest) {
  try {
    ...
    const paidState = parsePaidSignupStateFromObject(decodedState)
    ...
    if (!user) {
      ...
      if (existingUser) {
        ...
        return NextResponse.redirect(
          new URL(`${completionPath}?token=${pendingToken}&provider=twitter`, appOrigin)
        )
      }
    }
