...
export async function POST(request: NextRequest) {
  return runPatentApiRoute(request, '/api/v1/analysis/features', async () => {
    let body: any
    try {
    ...
    const analysis = await extractPublicInventionFeatures({ title: body.title, description: body.description })
    return {
      data: analysis,
