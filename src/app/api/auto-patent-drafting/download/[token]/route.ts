  ...
  try {
    ...
    return NextResponse.json({
      ...
      documents: documents.map((document: any) => ({
        ...
        downloadUrl: `/api/auto-patent-drafting/download/${params.token}/artifact/${document.id}`
      }))
    })
  } catch (error) {
    console.error('[AutoPatentDraftingDownload] Failed to fetch download metadata:', error)
