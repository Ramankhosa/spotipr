...
Users can cancel their own `QUEUED` or `PROCESSING` searches from novelty-search history. Cancellation is hard: the job is marked `CANCELLED`, the run is prevented from completing, report URLs are cleared, and public history/detail APIs report `CANCELLED` from the job state. An external request already in flight may return before the worker observes cancellation, but guarded stage writes must not mark the run completed, send completion email, or record completed usage for the cancelled job.
