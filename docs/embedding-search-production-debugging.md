...
- If deploy reports `P3009` for `20260619170000_optimize_local_patent_search`, deploy the corrected migration file, mark the failed attempt rolled back with `npx prisma migrate resolve --rolled-back 20260619170000_optimize_local_patent_search`, then rerun `npx prisma migrate deploy`. The original SQL used PostgreSQL's `STABLE` `array_to_string` function in an expression index; the corrected migration uses an immutable text-array wrapper. All index statements are idempotent, so retrying is safe if the failed attempt created earlier indexes.
