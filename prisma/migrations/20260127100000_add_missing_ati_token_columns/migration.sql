...
-- AddForeignKey (only if teams table and constraint don't already exist)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'teams')
       AND NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'ati_tokens_assignedTeamId_fkey')
    THEN
