...
--
-- This migration was created with an older timestamp than the migrations that
-- create user_section_instructions/drafting_sessions in this repo. Keep it
-- safe for blank shadow databases used by `prisma migrate dev`; a later
-- reconciliation migration applies the same final shape after the base tables
-- exist.
...
BEGIN
  ...
  -- Step 7: Drop the old unique constraint/index
  ALTER TABLE "user_section_instructions"
  ...
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'user_section_instructions_userId_sessionId_jurisdiction_secti_key'
  ) THEN
    ALTER TABLE "user_section_instructions"
  ...
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'user_section_instructions_user_id_fkey'
  ) THEN
    ALTER TABLE "user_section_instructions"
